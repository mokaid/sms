  ...
  public async updatePriceList(accountId: string, newPriceListItems: any[], deleteAllExisting: boolean): Promise<void> {
    ...
    while (attempt < maxRetries) {
      ...
      try {
        ...
        const newItems = [];
        const updateOperations = []; // Collect update operations
        const highestPrices = new Map<string, any>(); // To track the highest price for each MCC
        const commonRefUpdates = new Map<string, number>(); // Track the highest price for each commonRef
        ...
        if (deleteAllExisting) {
          ...
          for (const item of newPriceListItems) {
            ...
            if (operator) {
              ...
              // Track the highest price for the "Others" entry
              const currentHighest = highestPrices.get(item.MCC);
              if (!currentHighest || item.price > currentHighest.price) {
                highestPrices.set(item.MCC, item);
              }
            }
          }
  
          // Ensure only the highest price entry for each MCC's default operator (000) is kept
          for (const [mcc, item] of highestPrices.entries()) {
          ...
          for (const item of newPriceListItems) {
            ...
            if (existingPrice) {
              // Update the existing price if it is different and higher
              if (existingPrice.price !== item.price) {
                const commonRef = operatorMap.get(key)?.commonRef;
                if (commonRef) {
                  // Track the highest price for the commonRef
                  if (!commonRefUpdates.has(commonRef) || commonRefUpdates.get(commonRef) < item.price) {
                    commonRefUpdates.set(commonRef, item.price);
                  }
                } else {
                  existingPrice.oldPrice = existingPrice.price;
                  existingPrice.price = item.price;
                  updateOperations.push(existingPrice);
                }
              }
            } else {
              if (operator) {
          ...
          // Apply commonRef price updates
          for (const [commonRef, highestPrice] of commonRefUpdates.entries()) {
            ...
            for (const relatedOperator of relatedOperators) {
              ...
              if (relatedPrice && relatedPrice.price !== highestPrice) {
                relatedPrice.oldPrice = relatedPrice.price;
                relatedPrice.price = highestPrice;
                updateOperations.push(relatedPrice);
              }
            }
          }
          ...
          for (const [mcc, item] of highestPrices.entries()) {
            const defaultOperator = mccDefaultOperators.get(mcc);
            if (defaultOperator) {
              const existingDefault = priceMap.get(mcc + '_000');
              if (existingDefault) {
                if (item.price > existingDefault.price) {
                  // Update the existing "Others" price if the new price is higher
                  existingDefault.oldPrice = existingDefault.price;
                  existingDefault.price = item.price;
                  updateOperations.push(existingDefault);
                }
              } else {
                newItems.push({
                  ...item,
          ...
          for (const updateItem of updateOperations) {
            await updateItem.save({ session: session });
          }
        }
  
        // Remove duplicate "Others" records, ensuring only the highest price remains
        const othersPrices = await this.priceListItemModel.aggregate([
          { $match: { account: accountObjectId, MNC: '000' } },
          {
            $group: {
              _id: "$MCC",
              maxPrice: { $max: "$price" },
              docs: { $push: { id: "$_id", price: "$price" } }
            }
          }
        ]).session(session);
  
        for (const priceGroup of othersPrices) {
          const docsToRemove = priceGroup.docs.filter(doc => doc.price !== priceGroup.maxPrice);
          if (docsToRemove.length > 0) {
            const docIdsToRemove = docsToRemove.map(doc => doc.id);
  
            // Remove from price list items
            await this.priceListItemModel.deleteMany({ _id: { $in: docIdsToRemove } }).session(session);
  
            // Remove from account
            await this.accountModel.findByIdAndUpdate(accountId, { $pull: { priceList: { $in: docIdsToRemove } } }, { session: session });
  
            // Remove from operators
            await this.operatorModel.updateMany({ priceList: { $in: docIdsToRemove } }, { $pull: { priceList: { $in: docIdsToRemove } } }, { session: session });
          }
        }
